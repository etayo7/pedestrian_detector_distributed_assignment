...
class image_frame:
	...
	def color_detection_darknet(self, PD_loc, N, colours_value): #color detector [RGB]
		...
		# define different possible colours and their HSV ranges
	...
	def final_detection(self, PD_loc, id, colours):
		...
		for (xA, yA, xB, yB) in PD_loc:
			...
			rgb = self.hsv_to_rgb( float(colours_value[(id[i])*3])*2.0, 1.0, 1.0 )
...
def dk_find():
	...
	image = image_frame(n_robots, robot)
	...
	while not rospy.is_shutdown():
		...
		if (image.frame_received == 1) and (secs>t):
			...
			[image, id] = image.color_detection_darknet(PD_location, n_robots, colours_v) # assign each person to a target
