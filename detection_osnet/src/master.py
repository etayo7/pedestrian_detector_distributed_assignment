...
class Master:
    ...
    def __init__(self):
        ...
        if self.does_export:
            self.exportPath = f'~/PDDA_Results/Run_{datetime.datetime.now().strftime("%d-%m-%Y_%H-%M-%S")}'
        ...
        try:
            for id in range(self.robot_no):
                self.monitors.append(Monitor(bridge = bridge, colors = colors, ns = id + 1, target_no=rospy.get_param('/master/target_no'), source = rospy.get_param(f'/r_{i+1}/cfg/monitor/level')))
        except Exception:
