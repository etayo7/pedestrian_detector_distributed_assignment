...
# TARGETS DETECTOR NODE - dk_find (Arg 1 = N Robots, Arg 2 = robot id, Arg 3 = costf =f(blob=0/depth=1))
# (Args 4 = Colours of the targets HSV mean)

# Creator - Inigo Etayo
# Directors - Eduardo Montijano, Danilo Tardioli

# This node with four arguments receives the frames obtained through a
# RGBD camera. At each frame tries to detect pedestrians and knowing the colours of
# each target, is able to establish the id of each person detected. Then, using
# the depth of the camera or the size of the bounding box, this node sends to the
# assignment node the computed costs of each target detected, and the desired
# positions towards those targets.
...
# Load YOLO detecotr with tiny weights
...
class image_frame:
	...
	def __init__(self, N):
		# initialize strings for each publisher
	...
	def callback_depth(self, msg):  # read each depth frame
		self.img_depthS = msg
		return self

	def callback_assign(self, msg):  # read assignments
		id = msg.id - 1
		# self.assign = 0
		if id == r:
	...
	def color_detection_darknet(self, PD_loc, N, colours_value):  # color detector, target recognition
		imageFrame_orig = bridge.imgmsg_to_cv2(self.img_post, "bgr8")

		# initialize area for each target, N colours
		id = [0] * np.size(PD_loc, 0)
		i = 0

		# recognise each target detected in function of its color
		for (xA, yA, xB, yB) in PD_loc:
			...
			for color in range(N):
				HSV_dist[color] = math.sqrt((HSV_mean[0] - colours_value[color * 3]) ** 2 + (
	...
	def final_detection(self, PD_loc, id, colours):
		# Shows the frame of the camera with some information about the detection and the assignment
		image = bridge.imgmsg_to_cv2(self.img, "bgr8")
		height, width = image.shape[:2]
		colours_value = colours
		i = 0

		# draw the assignment at the top right corner
		xA = int(1 * width / 6)
		xA2 = xA + 7
		xB = width - 10
		yA = 10
		...
		if self.assign != 0:
			if (float(colours_value[(self.assign - 1) * 3 + 1])) > 100:
				rgb = colorsys.hsv_to_rgb(float(colours_value[(self.assign - 1) * 3]) / 180.0, 1.0, 1.0)
			else:
		...
		# draw the final bounding boxes of each detection with its colour
		for (xA, yA, xB, yB) in PD_loc:
			yA2 = yA - 25
	...
	def pixel2center(self, PD_loc, id, cost_function):
		# Compute the relative cost and the relative position of each target
		global timer
		...
		for (xA, yA, xB, yB) in PD_loc:
			...
			cost[id[i]] = 1.0
			# add argument, cost = f(bounding box size or depth central pixel)
			if c > 0 and cost_function == 1:  # f(depth central pixel)
				cost[id[i]] = (1 - (500 / c)) ** 3
			if c > 0 and cost_function == 0:  # f(bounding box size)
			...
			angle[(id[i] * 4) + 2] = c
			angle[(id[i] * 4) + 3] = d
			# if the target is almost centered, dont move the servos
			if x > (3 * width / 8) and x < (5 * width / 8) and y > (height / 3) and y < (2 * height / 3):
				...
				angle[(id[i] * 4) + 3] = 0
			i = i + 1
		# implementation to move the camera to the origin position if it detects no targets for 4 seconds
		if len(PD_loc) > 0:
	...
	def convert_depth_image(self, fil, col):
	# from Victor Fuertes TFM, given the coordinates and the depth of a pixel (the detected target center pixel)
	# computes the (x,y,z) relative 3D coordinates of that point, later used to move the servos towards that pixel
...
def dk_find():
	...
	while not rospy.is_shutdown():
		...
		if (image.frame_received == 1) and (secs > t):
			...
			# targets detection using darknet YOLO
			...
			# compute assignment depth-cost and
